...
function Search({ navBar }: forNavBar) {
  ...
  const isMediumScreen = useMediaQuery({ query: "(max-width: 60rem)" });
    ...
    <div
      className={
        ...
        (navBar
          ...
          : "rounded-full border-[1px] bg-[#f4f4f5] p-2 dark:bg-[#09090b]")
      }
    >
      <div className="flex items-center gap-4">
        {navBar || isMediumScreen ? (
          <Popover>
            <PopoverTrigger asChild>
              <Button
                variant={"outline"}
                className={`border-none text-center ${isMediumScreen ? "w-[200px] md:w-[200px]" : ""}`}
              >
                <p>Search</p>
              </Button>
            </PopoverTrigger>
            <PopoverContent className="mt-2 flex w-full flex-col gap-4 rounded-3xl lg:flex-row lg:rounded-full">
              <SearchDate
              ...
              <div className="w-full">
                <Button
                  variant="outline"
                  className="w-full bg-teal-500 text-white dark:text-[#09090b] md:h-full"
                >
                  <Link
